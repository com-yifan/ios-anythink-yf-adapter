...
#define Native_Express_PlacementID @"b69ccd807a681e"
...
//注意模版的宽高比与后台选择对应
#define ExpressAdWidth (kScreenW-30)
//注意模版的宽高比与后台选择对应
#define ExpressAdHeight (ExpressAdWidth/2.f)

#define ExpressMediaViewWidth (ExpressAdWidth)
#define ExpressMediaViewHeight (350 - kNavigationBarHeight - 150)

- (CGSize)displaySizeForOffer:(ATNativeAdOffer *)offer fallback:(CGSize)fallbackSize {
    CGFloat width = offer.nativeAd.nativeExpressAdViewWidth > 0.0f ? offer.nativeAd.nativeExpressAdViewWidth : fallbackSize.width;
    CGFloat height = offer.nativeAd.nativeExpressAdViewHeight > 0.0f ? offer.nativeAd.nativeExpressAdViewHeight : fallbackSize.height;
...
//    ATDemoLog(@"checkLoadStatus : %d",status.isLoading);
    ...
    CGSize displaySize = [self displaySizeForOffer:offer fallback:CGSizeMake(ExpressAdWidth, ExpressAdHeight)];
    AdDisplayVC *showVc = [[AdDisplayVC alloc] initWithOffer:offer adViewSize:displaySize];
    ...
    //设置展示根控制器
    config.rootViewController = showVc;
    ...
    //展示广告
    [showVc bindAdView:nativeADView];
