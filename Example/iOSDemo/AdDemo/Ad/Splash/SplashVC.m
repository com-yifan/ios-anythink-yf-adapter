...
#define SplashPlacementID @"b659e4951118f4"
...
- (UIWindow *)splashWindow {
    if (!_splashWindow) {
        ...
        window.windowLevel = UIWindowLevelAlert + CGFLOAT_MAX;
...
- (void)showFeedBackBtn {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [closeBtn addTarget:self action:@selector(closeSplashAd) forControlEvents:(UIControlEventTouchUpInside)];
        closeBtn.backgroundColor = [UIColor blueColor];
        closeBtn.frame = CGRectMake(0, 100, 100, 100);
        [self.splashWindow addSubview:closeBtn];
    });
}

- (void)closeSplashAd {
    [[ATAdManager sharedManager] customCloseADEventWithPlacementID:SplashPlacementID];
}
